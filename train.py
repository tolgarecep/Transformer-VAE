...
"""
Transformer based:
python train.py --train data/yelp/train.txt --valid data/yelp/valid.txt --save_dir checkpoints/transformer --model_type transformer
LSTM based:
python train.py --train data/yelp/train.txt --valid data/yelp/valid.txt --save_dir checkpoints/lstm --model_type lstm
...
# VAE arguments
parser.add_argument('--vocab_size', type=int, default=50000, metavar='N',
                    help='keep N most frequent words in vocabulary')
parser.add_argument('--model_type', default='transformer', metavar='M',
                    choices=['lstm', 'transformer'],
                    help='which model to learn')
parser.add_argument('--dim_z', type=int, default=64, metavar='D',
                    help='dimension of latent variable z')
parser.add_argument('--dim_emb', type=int, default=1024, metavar='D',
...
parser.add_argument('--pe_max_len', type=int, default=5000, metavar='L',
    ...
    with torch.no_grad():
        if args.model_type == 'transformer':
            ...
            for k, v in losses.items():
                meters[k].update(v.item(), enc_inputs.size(1))
        else:
...
def main(args):
    ...
    train_batches, _ = get_tokenized_batches(train_sents, vocab, args.model_type, args.batch_size, device)
    valid_batches, _ = get_tokenized_batches(valid_sents, vocab, args.model_type, args.batch_size, device)
    best_val_loss = None
