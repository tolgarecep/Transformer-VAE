...
from models import LSTM_VAE, T_VAE
from corpus_to_tokenized import batchify
...
def main(args):
    ...
    # Prepare data
    train_batches = batchify(args.train, args.batch_size)
    logging('# train sents {}'.format(len(train_sents)*args.batch_size), log_file)
    valid_batches = batchify(args.valid, args.batch_size)
    logging('# valid sents {}'.format(len(valid_sents)*args.batch_size), log_file)
    ...
    model = {'lstm': LSTM_VAE, 'transformer': T_VAE}[args.model_type](
